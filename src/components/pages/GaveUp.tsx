...
const GaveUp = () => {
    ...
    // Fetch total count for "GaveUp" books
    useEffect(() => {
        const fetchCount = async () => {
            try {
                const token = localStorage.getItem('authToken');
                const response = await fetch(
                    `http://localhost:8080/api/books/status/gave_up/count?search=${encodeURIComponent(searchTerm)}`,
                    {
                        headers: {
                            'Authorization': `Bearer ${token}`
    ...
    const handleDelete = async (bookId: number) => {
        if(window.confirm('Are you sure you want to delete this book?')) {
            try {
                ...
                // Also refresh count
                const countResponse = await fetch('http://localhost:8080/api/books/status/gave_up/count',
                    {
                        headers: {
                            'Authorization': `Bearer ${token}`
                        }
                ...
                <div className="border-2 border-none p-8 rounded-lg h-full">
                    <div className="mb-8">
                        <h1 className="text-4xl font-bold text-amber-800 flex items-center justify-center px-4 relative">
                            <span className="text-center">Gave up</span>
                        </h1>
                        ...
                        {books.length === 0 ? (
                            <div className="col-span-full text-center py-8">
                                <p className="text-amber-800 text-lg">
                                    {searchTerm ? `No books found for "${searchTerm}"` : "No books in your gave up list yet."}
                                </p>
                            </div>
                        ) : (
                            books.map(book => (
